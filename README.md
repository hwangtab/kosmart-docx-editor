...
### 🔍 문서 구조 탐색
* **`find_table_under_heading` (제목 기반 스마트 테이블 추적기)**: 인간의 눈처럼 문서를 위에서 아래로 순서대로 스캔합니다. 사용자가 지정한 '특정 제목 텍스트'를 발견하면, **그 제목 바로 아래에 등장하는 첫 번째 표(Table)**가 문서 전체에서 몇 번째 인덱스(table_index)인지 똑똑하게 찾아내어 미리보기와 함께 반환합니다. 더 이상 허공에 텍스트를 잘못 쏟아붓는 실수를 예방할 수 있습니다.
...
* **`safe_replace_docx_table_keyword` (특정 표 안방 치환)**: 오직 지정한 N번째 표(Table) 내부에서만 단어를 찾아냅니다. 엉뚱한 표나 본문의 내용을 실수로 건드리는 대참사를 막아줍니다.
* **`safe_replace_docx_cell` (안전 셀 덮어쓰기)**: 워드 안에 숨어있는 보이지 않는 태그 서식을 보존하면서 내용물 텍스트만 덮어써 줍니다.
  * 🛡️ **안전 장치 탑재! (expected_old_text)**: 파라미터로 "이 칸에는 기존에 '홍길동'이란 글씨가 있어야 해"라고 기대값을 넣어두면, 엉뚱한 셀에 잘못 찾아갔을 때 덮어쓰기를 자동 취소하고 문서를 보호합니다.
...
* **`copy_docx_table` (표 전체 복사)**: 문서 내 기존 표 형식을 그대로 본따서 바로 아래에 원본과 동일한 빈 표를 고스란히 복제해 줍니다. 행/열 추가로 부족할 때 표 공간 자체를 증설합니다.
* **`delete_docx_table` (표 통째로 삭제)**: 문서 내 불필요한 표 덩어리를 통째로 깔끔하게 날려버립니다.
* **`insert_docx_table_row` (안전 행 원격 삽입)**: 지정된 표의 특정 위치에 복제된 행(Row)을 안전하게 삽입합니다. **순수 ZIP+XML 파싱 기술**을 적용하여, 일반 파이썬 라이브러리가 병합 셀(Merged Cells)을 만났을 때 빠지는 **무한 루프 버그를 원천 차단**했습니다.
* **`delete_docx_table_row` (안전 행 원격 삭제)**: 표 안에서 잘못 들어갔거나 필요 없는 특정 줄(Row) 하나를 감쪽같이 지웁니다. 이 역시 순수 ZIP+XML 방식으로 재설계되어, 어떤 복잡한 표에서도 뻗지 않고 초고속으로 삭제합니다.
