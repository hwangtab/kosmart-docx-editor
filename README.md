# 🪄 마법의 무결점 워드 통합 에디터 (DOCX MCP Server)

안녕하세요! 이 도구는 한국스마트협동조합 실무를 위해 특수 제작된 **DOCX(워드/한글 변환 문서) 전용 초강력 AI 플러그인(MCP)** 입니다. 

평소에 AI(클로드, 커서 등)에게 "이 워드 제안서 내용을 이렇게 고쳐줘"라고 시켰을 때, AI가 문서 서식(볼드체, 색상)을 전부 파괴하거나 표와 텍스트 상자를 망가뜨려 문서가 열리지 않는 경험을 해보셨나요? 

이제 이 툴박스를 **복사+붙여넣기 한 번**으로 내 AI에 꽂아주기만 하면, 복잡하게 병합된 **지옥의 엑셀 표**부터 **이미지 교체**, **머리글/바닥글**, **하이퍼링크**, 심지어 파이썬 기본 API로는 불가능했던 둥둥 떠 있는 **투명 텍스트 상자(Floating Shapes)** 내부까지! 문서의 레이아웃을 1mm도 망가뜨리지 않고 AI가 척척 수정해 줍니다! 😎
